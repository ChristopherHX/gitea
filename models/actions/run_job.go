...
// ParseWorkflow parses the workflow structure from the ActionRunJob.WorkflowPayload
func (job *ActionRunJob) ParseWorkflow() (*jobparser.SingleWorkflow, error) {
...
func (job *ActionRunJob) ParseJob() (*jobparser.Job, error) {
	parsedWorkflow, err := job.ParseWorkflow()
	if err != nil {
		return nil, err
	}
	_, workflowJob := parsedWorkflow.Job()
	if workflowJob == nil {
